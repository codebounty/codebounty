...
(function (undefined) {
    var btcSymbol = "\
        ...
        <g>\
            <g>\
                <path fill='#FFF' d='M217.021,167.042c18.631-9.483,30.288-26.184,27.565-54.007c-3.667-38.023-36.526-50.773-78.006-54.404l-0.008-52.741\
    ...
    var ui = {
        ...
        setupPostBounty: function (initValue) {
            ui.render(function () {
                ...
                var usdDiv = "" +
                    "<div class='usd inputWrapper'><label for='bountyInput' class='bountyCurrency currencySymbol'>$</label>" +
                ...
                // Generate a Bitcoin address for this issue/user pair.
                // TODO: Move this inside the toggle code?
                messenger.sendMessage(
                    {
                        method: "btcAddressForIssue",
                        params: [thisIssueUrl]
                    },
                    ...
                    function (message) {
                        ...
                        if (message.result) {
                            var btcDiv = "" +
                                "<div class='btc inputWrapper'>" +
                                "<div class='qrCode' data='" + message.result + "'></div>" +
                                "<div class='btcAddress'><a href='bitcoin:" + message.result + "'>" + message.result + "</a></div>" +
                                "</div>";
                            $(btcDiv).insertAfter(ui._container);
                            
                            $('.qrCode').each(function() {
                                var $this = $(this);
                                var size = Math.min(82, $this.parent().width());
                                $this.qrcode({
                                  width: size,
                                  height: size,
                                  text: $this.attr("data")
                                ...
                                $('canvas', $this).wrap(
                                    $('<a>').attr('href', 'bitcoin:' + $this.attr("data")));
                            });
                        }
                    }
                );
                ...
                .on('toggle', function (e, usd) {
                    $(".usd.inputWrapper, #addBounty").toggle(usd);
                    $(".btc.inputWrapper").toggle(!usd);
                    $("#currencyInput").val(usd ? "usd" : "btc");
                    //$(".currencySymbol").html(usd ? "$" : btcSymbol);
                })
                .toggles({
                    animate: 0,
                    type: "select", 
